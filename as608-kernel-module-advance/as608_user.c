...
static sem_t *sem; /* POSIX semaphore for access control */
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "--exec") == 0) {
            ...
            if (pid == 0) {
                execlp("echo", "echo", "Processing fingerprint data", NULL);
                perror("Exec failed");
                exit(1);
            }
        } else if (strcmp(argv[1], "--ioctl") == 0 && argc > 2) {
            ioctl_cmd = argv[2];
        }
    }
    ...
    if (!ioctl_cmd || strcmp(ioctl_cmd, "UPLOAD_FLASH_FEATURE") == 0) {
        ...
        if (ioctl(fd, AS608_IOCTL_UPLOAD_FLASH_FEATURE, feature_data) < 0) perror("IOCTL UPLOAD_FLASH_FEATURE failed");
            ...
            printf("User: Reading from shared memory\n");
            sem_wait(sem);
            memcpy(feature_data->buffer, shm_buf, feature_data->len);
            sem_post(sem);
        }
        sem_post(sem);
    }
    ...
    if (!ioctl_cmd || strcmp(ioctl_cmd, "UPLOAD_IMAGE_FEATURE") == 0) {
        sem_wait(sem);
        if (ioctl(fd, AS608_IOCTL_UPLOAD_IMAGE_FEATURE, feature_data) < 0) perror("IOCTL UPLOAD_IMAGE_FEATURE failed");
        else {
            printf("User: Reading from shared memory\n");
            sem_wait(sem);
            memcpy(feature_data->buffer, shm_buf, feature_data->len);
    ...
    if (!ioctl_cmd || strcmp(ioctl_cmd, "DOWNLOAD_FLASH_FEATURE") == 0) {
        sem_wait(sem);
        if (ioctl(fd, AS608_IOCTL_DOWNLOAD_FLASH_FEATURE, feature_data) < 0) perror("IOCTL DOWNLOAD_FLASH_FEATURE failed");
        sem_post(sem);
    }
    ...
    if (!ioctl_cmd || strcmp(ioctl_cmd, "UPLOAD_IMAGE") == 0) {
        ...
        if (ioctl(fd, AS608_IOCTL_UPLOAD_IMAGE, image_data) < 0) perror("IOCTL UPLOAD_IMAGE failed");
            ...
            printf("User: Reading from shared memory\n");
            sem_wait(sem);
            memcpy(image_data->buffer, shm_buf, image_data->len);
            sem_post(sem);
        }
        sem_post(sem);
    }
    ...
    if (!ioctl_cmd || strcmp(ioctl_cmd, "DOWNLOAD_IMAGE") == 0) {
        sem_wait(sem);
        if (ioctl(fd, AS608_IOCTL_DOWNLOAD_IMAGE, image_data) < 0) perror("IOCTL DOWNLOAD_IMAGE failed");
        sem_post(sem);
    }
